  ...
  <div class="application-page">
    <section class="page-top">
      <div class="page-top-main">
        <p class="eyebrow">检验医生 · 检验申请</p>
        <h1>查看检验申请</h1>
        <p>仅展示 LAB 类检验申请，支持筛选、分配房间、处理和报告查看。</p>
        <div class="hero-tags">
...
const labList = computed(() => allList.value.filter((item) => item.itemCategory === 'LAB'))
...
async function handlePrimaryAction(row: InspectionOrderVo) {
  ...
  if (row.status === 'IN_PROCESS') {
    openReportWorkspace(row, orderItemId)
    return
  }
  ...
  try {
    ...
    if (row.status === 'WAITING_ASSIGN' || row.status === 'QUEUED') {
      ...
      ElMessage.success('已开始处理，请填写检验报告')
    }
    openReportWorkspace(row, orderItemId)
  } catch (error: any) {
...
async function assignOrder(row: InspectionOrderVo, refreshAfterAssign = true) {
  const orderItemId = getPrimaryOrderItemId(row)
  if (!orderItemId) {
    ElMessage.warning('缺少检验明细ID，无法分配')
    return false
  }
...
function openTaskDetail(row: InspectionOrderVo) {
  const orderItemId = getPrimaryOrderItemId(row)
  if (!orderItemId) {
    ElMessage.warning('缺少检验明细ID，无法进入任务处理')
    return
  }
...
function openReportWorkspace(row: InspectionOrderVo, orderItemId: string) {
  router.push({
    path: '/examination-doctor/report',
    query: {
      orderItemId,
