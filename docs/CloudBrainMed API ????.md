...
*当前 \`JwtAuthenticationFilter\`、\`RateLimitFilter\`、\`RequestLogFilter\` 尚无有效实现；除检查医生 CT 内部代理端点外，管理员 MLOps 接口已在 `ai-service` 的 `MlOpsController` 内校验 token，因此不依赖网关完成管理员身份保护。*
...
|/admin-service/ml/\*\*|管理员|`MlOpsController` 校验管理员 token|
...
**模块职责：** 由 `ai-service` 的 `MlOpsController` 负责 AI 推理统计、日志审计、训练样本、模型训练和运行状态检查。管理端前端当前只保留 AI 推理看板，不再提供独立的“样本标注”和“模型管理”页面；后端接口继续保留兼容。



**路由范围：** `/admin-service/ml/**`



**权限说明：** 仅管理员访问；`MlOpsController` 在所有映射执行前校验 JWT，支持 `Authorization: Bearer` 和兼容请求头 `token`。token 缺失、非法或过期返回 `code=401`，合法 token 但 `roleType != 3` 返回 `code=403`。
...
|接口名称|请求方式|接口地址|
|---|---|---|
|推理统计|`GET`|`/admin-service/ml/dashboard/inference-stats`|
|模型统计|`GET`|`/admin-service/ml/dashboard/model-stats`|
|推理日志分页|`GET`|`/admin-service/ml/inference/logs`|
|训练样本分页|`GET`|`/admin-service/ml/samples/list`|
|更新训练样本标注|`POST`|`/admin-service/ml/samples/update`|
|模型列表|`GET`|`/admin-service/ml/models/list`|
|触发模型训练|`POST`|`/admin-service/ml/models/train`|
|训练任务列表|`GET`|`/admin-service/ml/models/tasks`|
|Python推理服务健康检查|`GET`|`/admin-service/ml/python/health`|
...
>
> 为保持 doctor-service 到 ai-service 的既有内部代理契约，上述六个 `/admin-service/ml/inference/ct-*` 端点不执行管理员前置鉴权；其余 MLOps 管理接口均要求管理员 token。
...
|项目|内容|
|---|---|
|接口地址|`/admin-service/ml/dashboard/inference-stats`|
|请求方式|`GET`|
|请求头|`Authorization: Bearer <管理员 JWT>`；兼容 `token: <管理员 JWT>`|
|权限说明|仅管理员；Controller 校验管理员 token|
...
```HTTP
GET /admin-service/ml/dashboard/inference-stats
无请求体
```
...
|项目|内容|
|---|---|
|接口地址|`/admin-service/ml/dashboard/model-stats`|
|请求方式|`GET`|
|请求头|`Authorization: Bearer <管理员 JWT>`；兼容 `token: <管理员 JWT>`|
|权限说明|仅管理员；Controller 校验管理员 token|
...
```HTTP
GET /admin-service/ml/dashboard/model-stats
无请求体
```
...
|项目|内容|
|---|---|
|接口地址|`/admin-service/ml/inference/logs`|
|请求方式|`GET`|
|请求头|`Authorization: Bearer <管理员 JWT>`；兼容 `token: <管理员 JWT>`|
|权限说明|仅管理员；Controller 校验管理员 token|
...
```HTTP
GET /admin-service/ml/inference/logs
无请求体
```
...
|项目|内容|
|---|---|
|接口地址|`/admin-service/ml/samples/list`|
|请求方式|`GET`|
|请求头|`Authorization: Bearer <管理员 JWT>`；兼容 `token: <管理员 JWT>`|
|权限说明|仅管理员；Controller 校验管理员 token|
...
```HTTP
GET /admin-service/ml/samples/list
无请求体
```
...
|项目|内容|
|---|---|
|接口地址|`/admin-service/ml/samples/update`|
|请求方式|`POST`|
|请求头|`Authorization: Bearer <管理员 JWT>`；兼容 `token: <管理员 JWT>`|
|权限说明|仅管理员；Controller 校验管理员 token|
...
|项目|内容|
|---|---|
|接口地址|`/admin-service/ml/models/list`|
|请求方式|`GET`|
|请求头|`Authorization: Bearer <管理员 JWT>`；兼容 `token: <管理员 JWT>`|
|权限说明|仅管理员；Controller 校验管理员 token|
...
```HTTP
GET /admin-service/ml/models/list
无请求体
```
...
|项目|内容|
|---|---|
|接口地址|`/admin-service/ml/models/train`|
|请求方式|`POST`|
|请求头|`Authorization: Bearer <管理员 JWT>`；兼容 `token: <管理员 JWT>`|
|权限说明|仅管理员；Controller 校验管理员 token|
...
|项目|内容|
|---|---|
|接口地址|`/admin-service/ml/models/tasks`|
|请求方式|`GET`|
|请求头|`Authorization: Bearer <管理员 JWT>`；兼容 `token: <管理员 JWT>`|
|权限说明|仅管理员；Controller 校验管理员 token|
...
```HTTP
GET /admin-service/ml/models/tasks
无请求体
```
...
|项目|内容|
|---|---|
|接口地址|`/admin-service/ml/python/health`|
|请求方式|`GET`|
|请求头|`Authorization: Bearer <管理员 JWT>`；兼容 `token: <管理员 JWT>`|
|权限说明|仅管理员；Controller 校验管理员 token|
...
```HTTP
GET /admin-service/ml/python/health
无请求体
```
...
`admin-service` 提供管理端首页概览。该接口只统计管理员模块已经管理的医生、科室、排班和药品数据，不读取检查/检验共享队列，也不写入任何业务表。

#### 2\.8\.1 查询管理端首页概览

|项目|内容|
|---|---|
|接口地址|`/admin-service/dashboard/overview`|
|请求方式|`GET`|
|请求头|`token: 管理员 JWT`|
|权限说明|仅管理员；token 缺失、非法或过期返回 `code=401`，合法 token 但 `roleType != 3` 返回 `code=403`|
...
|参数名|类型|说明|
|---|---|---|
|doctorCount|Integer|未软删除的医生账号总数，来源于 `doctor.is_deleted=0`|
|departmentCount|Integer|启用科室总数，来源于 `department.status=1`|
|todayScheduleCount|Integer|当天有效且已发布的排班记录数，来源于 `doctor_schedule.status=1` 且 `schedule_status=PUBLISHED`|
|medicineCount|Integer|药品记录总数，来源于 `medicine`|
...
**前端规则：** 管理端首页加载该接口展示真实概览数据；加载失败时保留 `--`，不显示伪造统计值。管理端只保留“AI 推理看板”入口，不再展示“样本标注”和“模型管理”菜单或页面。AI 推理看板仍通过 `ai-service` 的 `/admin-service/ml/dashboard/inference-stats` 与 `/admin-service/ml/model/list` 加载推理统计和已注册模型。
...
|管理员账号、排班、药品接口|保留现有实现|对应 `admin-service` Controller 已有映射；本次不修改这些接口|
|管理端数据看板|已纳入|`admin-service` 已实现 `/admin-service/dashboard/overview`，见 2.8|
