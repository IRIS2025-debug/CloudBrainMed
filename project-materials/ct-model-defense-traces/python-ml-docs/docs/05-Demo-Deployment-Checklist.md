...
- `Model/weights/best_attention_adamw.pth` 存在，默认 `MODEL_TYPE=attention`。
- 上传 CT 文件后，检查医生端 CT 推理页能返回 mask 结果。
- mask 下载链接走 Java 后端代理，不直接暴露 Python 服务地址。
- 返回 JSON 包含 `artifactDetected`、`positivePixels`、`totalPixels`、`artifactRatio`、`modelType`、`modelVersion`、`reportInput`，其中 `reportInput` 供后续大语言模型生成文字描述和报告初稿。
...
- 检查医生端：CT 伪影检测上传、结构化结果展示、mask 下载。
- 管理员端：个人信息编辑、头像上传、修改密码。
- 管理员端：MLOps Dashboard、模型列表、训练任务、样本标注。
...
```powershell
cd backEnd/services/ai-service/python-ml
python -m unittest tests.test_ct_artifact_result
```
...
```powershell
cd backEnd
.\mvnw.cmd -pl services/ai-service -Dtest=InferenceEngineTest test
```
